...
declare global {
  namespace jspdf {
    interface jsPDF {
      ...
      // Fix: Add missing jsPDF properties to the interface to resolve type errors.
      internal: any;
...
export const generatePdfReport = async (issues: Issue[]): Promise<void> => {
  ...
  // The autoTable plugin should add the method to the doc instance's prototype.
  // When this fails (a common issue with UMD modules), we cast to `any` to call it.
  // This resolves the "doc.autoTable is not a function" runtime error.
...
export const generatePptxReport = async (issues: Issue[]): Promise<void> => {
  // Handle UMD module loading where the constructor might be on the `.default` property.
  // This fixes the "window.PptxgenJS is not a constructor" error.
  const PptxgenJS_ = (window.PptxgenJS as any).default || window.PptxgenJS;
