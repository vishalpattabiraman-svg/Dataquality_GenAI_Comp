...
const BusinessRulesViolations: React.FC<BusinessRulesViolationsProps> = ({ issues }) => {
    ...
    return (
        <div>
            <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">Business Rule Violations</h3>
            <div className="space-y-4">
                {sortedTableNames.map(tableName => (
                    <div key={tableName} className="bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
                        <div className="p-4 border-b border-gray-200 dark:border-gray-700/50">
                            <div className="flex items-center space-x-3">
                                <TableIcon className="h-6 w-6 text-brand-primary dark:text-brand-secondary flex-shrink-0" />
                                <h4 className="text-lg font-semibold text-gray-800 dark:text-white truncate" title={tableName}>
                        ...
                        <div className="p-4 space-y-3">
                            {issuesByTable[tableName].map((issue, index) => (
                                <div key={index} className="flex items-start justify-between gap-4 p-3 bg-gray-50 dark:bg-gray-900/50 rounded-md">
